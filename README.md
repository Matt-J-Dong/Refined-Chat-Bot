...
## Methodology
In our research, it was clear to see that the evaluation of chat bots were fairly subjective and sometimes tested capabilities that would be outside the scope of our model. Although not perfect for our model, the SuperGLUE benchmark by Wang et. al is a series of tests utilizing 8 different metrics in order to create a final score. The categories we would specifically look at would be the BoolQ question answering task, in which the model is given a short passage and a yes or no question, and the MultiRC, in which the model is given a paragraph and a multiple choice question. On top of these metrics, we would utilize a general accuracy metric and the fall back rate, FBR, which is the rate at which the model simply answers, "I don't know" or some variation of that response. In addition to all of these quantitative metrics, we are also looking to sample random Harry Potter fans in order to assess our model on a more subjective basis.
